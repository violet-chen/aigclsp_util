...
class SaveSafetyImage:
    ...
    def nsfw_checker(self, images, filename_prefix="ComfyUI",sensitivity=0.5):
        try:
            ...
            # 保存图片
            nsfw_list = list(nsfw) if isinstance(nsfw,list) else [nsfw] # nsfw列表
            filename_prefix += self.prefix_append
            full_output_folder, filename, counter, subfolder, filename_prefix = folder_paths.get_save_image_path(filename_prefix, self.output_dir, images[0].shape[1], images[0].shape[0])
            results = list()
            for (batch_number, image) in enumerate(checked_image):
                i = 255. * image.cpu().numpy()
                img = Image.fromarray(np.clip(i, 0, 255).astype(np.uint8))
                filename_with_batch_num = filename.replace("%batch_num%", str(batch_number))
                file = f"{filename_with_batch_num}_{counter:05}_.png"
                if nsfw_list[batch_number]:
                    results.append({
...
class RaiseNode:
    ...
    def INPUT_TYPES(cls):
        inputs = {
            "required":{
                ...
                "do_raise": ("BOOLEAN",{"default": False}),
            },
        }
        return inputs
    ...
    def do_raise(self, value,message,do_raise):
        if do_raise:
            raise Exception(message)
        return value,

NODE_CLASS_MAPPINGS  = {
    "SaveSafetyImage":SaveSafetyImage,
    "RaiseNode":RaiseNode,
    }
NODE_DISPLAY_NAME_MAPPINGS  = {
    "SaveSafetyImage":"aigclsp save safety image",
    "RaiseNode":"aigclsp raise node"
    }